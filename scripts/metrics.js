...
function runMetricsTests() {
    ...
    assert('Reset: WPM is 0', wpmEl && wpmEl.textContent === '0', 0, wpmEl?.textContent);
    assert('Reset: CPM is 0', cpmEl && cpmEl.textContent === '0', 0, cpmEl?.textContent);
    assert('Reset: Accuracy is 0%', accEl && accEl.textContent === '0%', '0%', accEl?.textContent);
    assert('Reset: Mistakes is 0', misEl && misEl.textContent === '0', 0, misEl?.textContent);
    ...
    const hasShape = typeof metrics.wpm === 'number' && typeof metrics.cpm === 'number' &&
        typeof metrics.accuracy === 'number' && typeof metrics.mistakes === 'number' &&
        typeof metrics.totalCharacters === 'number' && typeof metrics.correctCharacters === 'number';
    assert('getMetrics() returns valid object with all fields', hasShape, true, hasShape);
    assert('getMetrics() mistakes count', metrics.mistakes === 1, 1, metrics.mistakes);

    // Reset for normal use
    metricsCalculator.reset();

    // Log summary
    console.log('%c--- Metrics.js Test Results ---', 'font-weight: bold; font-size: 14px');
    results.forEach((r) => {
        const style = r.status === 'PASS' ? 'color: green' : 'color: red';
        console.log(`%c${r.status} ${r.test}`, style, r.expected !== undefined ? `(expected: ${r.expected}, got: ${r.actual})` : '');
    });
    console.log(`%cTotal: ${passed} passed, ${failed} failed`, 'font-weight: bold', passed + failed, 'tests');
